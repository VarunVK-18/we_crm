...
// Submit 360° Compliance Form
exports.submit360ComplianceForm = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Process uploaded files if any
    const files = req.files || []; // upload.any() provides an array of files in req.files
    const uploadedDocs = [];
    ...
    const updatedDetails = {
      ...order.details,
      compliance360Form: formData,
      compliance360Docs: uploadedDocs,
    };
    ...
    res.status(200).json({ message: '360° Compliance form submitted successfully!', order });
  } catch (error) {
    console.error('Error submitting 360° Compliance form:', error);
    res.status(500).json({ message: 'Server error while submitting 360° Compliance form.', error: error.message });
  }
};
