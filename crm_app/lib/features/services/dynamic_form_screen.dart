...
class _DynamicFormScreenState extends ConsumerState<DynamicFormScreen> {
  ...
  List<TextInputFormatter> _getFormatters(FormFieldSchema field) {
    ...
    bool hasPan = RegExp(r'\bpan\b').hasMatch(lowerName) ||
        RegExp(r'\bpan\b').hasMatch(lowerLabel);
    bool isNotNameOrDate =
        !RegExp(r'name|date|dob|first|last').hasMatch(lowerName) &&
            !RegExp(r'name|date|dob|first|last').hasMatch(lowerLabel);

    if ((hasPan && isNotNameOrDate) ||
        lowerName.contains('gstin') ||
        lowerLabel.contains('gstin') ||
        lowerLabel.contains('gst number') ||
        lowerName == 'cin' ||
        lowerLabel.contains('cin') ||
        lowerName.contains('ifsc') ||
        lowerLabel.contains('ifsc') ||
        lowerName.contains('tan') ||
        lowerLabel.contains('tan') ||
        lowerName.contains('lei') ||
        lowerLabel.contains('lei')) {
    ...
    } else if (lowerName.contains('pin') ||
        lowerLabel.contains('pin code') ||
        lowerName.contains('postal')) {
      formatters.add(FilteringTextInputFormatter.digitsOnly);
  ...
  Future<void> _fetchSchema() async {
    try {
      final response = await http.get(Uri.parse(
          '$kBaseUrl/api/forms/service/${Uri.encodeComponent(widget.order.serviceType)}'));
      if (response.statusCode == 200) {
        ...
        // Fetch prefill data
        try {
          final prefillResponse = await http.get(
              Uri.parse('$kBaseUrl/api/orders/${widget.order.id}/prefill'));
      ...
      showGlobalError(e);
      if (mounted)
        _showError('Could not load the dynamic form. Please try again later.');
    } finally {
  ...
  Future<void> _loadDraft() async {
    ...
    final draft = await draftService.loadDraft(
        widget.order.id, 'DynamicForm_${widget.order.serviceType}');
  ...
  Future<void> _saveDraft() async {
    ...
    await draftService.saveDraft(
        widget.order.id, 'DynamicForm_${widget.order.serviceType}', data);
  }
    ...
    for (var field in fields) {
      ...
      if (field.type == 'group') {
        ...
        if (field.arrayConfig?.dynamicCountRef != null) {
          final dynCount =
              widget.order.details[field.arrayConfig!.dynamicCountRef!];
          if (dynCount != null) {
      ...
      } else if (['text', 'number', 'email', 'phone', 'date']
          .contains(field.type)) {
        ...
        String? autofillValue;
        if (userProfile.containsKey(currentPath) &&
            field.options.contains(userProfile[currentPath])) {
          autofillValue = userProfile[currentPath].toString();
        } else if (userProfile.containsKey(field.name) &&
            field.options.contains(userProfile[field.name])) {
          autofillValue = userProfile[field.name].toString();
        } else {
  ...
  Future<void> _pickFile(
      String pathKey, List<String>? allowedExtensions) async {
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        if (['jpg', 'jpeg', 'png', 'pdf'].contains(ext)) {
          ...
          final res = await ocrService.validateDocument(
              result.files.single.path!, pathKey);
          ...
          if (res['success'] == true) {
            ...
            showDialog(
                ...
                builder: (context) => AlertDialog(
                      ...
                      content: Text(res['message'] ?? 'Invalid document.',
                          textAlign: TextAlign.center),
                      actions: [
                        SizedBox(
  ...
  Future<void> _submitDetails() async {
      ...
      for (var cv in _schema!.crossValidations) {
        if (cv.type == 'sumEquals') {
          ...
          if (sum != double.tryParse(cv.value.toString())) {
            _showError(cv.message ??
                'Cross validation failed: sum is not ${cv.value}');
            return;
          }
        }
      }
    ...
    void validateFiles(List<FormFieldSchema> fields, String parentPath) {
      for (var field in fields) {
        ...
        if (field.type == 'file' && field.required) {
          bool hasValidNewFile = _filePaths[currentPath] != null &&
              _filePaths[currentPath]!.isNotEmpty;
          ...
          if (!hasValidNewFile) {
            ...
            if (lowerFieldName.contains('pan'))
              targetDocType = 'pan';
            else if (lowerFieldName.contains('aadhaar'))
              targetDocType = 'aadhaar';
            else if (lowerFieldName.contains('incorporation') ||
                lowerFieldName.contains('coi'))
              targetDocType = 'coi';
            else if (lowerFieldName.contains('address') ||
                lowerFieldName.contains('electricity'))
              targetDocType = 'addressProof';
            else if (lowerFieldName.contains('photo'))
              targetDocType = 'directorPhoto';
            else if (lowerFieldName.contains('bank'))
              targetDocType = 'bankStatement';
            else if (lowerFieldName.contains('gst'))
              targetDocType = 'gst';
            else if (lowerFieldName.contains('moa'))
              targetDocType = 'moa';
            else if (lowerFieldName.contains('aoa'))
              targetDocType = 'aoa';
            else if (lowerFieldName.contains('sales'))
              targetDocType = 'salesInvoice';
            else if (lowerFieldName.contains('purchase'))
              targetDocType = 'purchaseBills';

            dynamic existingDoc;
            if (targetDocType.isNotEmpty) {
              existingDoc = _prefillDocs.firstWhere(
                  (doc) => doc['documentType'] == targetDocType,
                  orElse: () => null);
            }
    ...
    try {
      ...
      final uri = Uri.parse(
          '$kBaseUrl/api/orders/${widget.order.id}/submit-dynamic-form');
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ...field.subFields
                .map((subField) => _buildField(subField, currentPath)),
          ],
        ),
      );
    } else if (field.type == 'array') {
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  ...field.subFields.map(
                      (subField) => _buildField(subField, '$currentPath[$i]')),
                ],
              ),
            ),
        ],
  ...
  Widget _buildTextField(FormFieldSchema field, String pathKey) {
    ...
    if (keyboardType == TextInputType.phone) {
      return PhoneInputField(
        ...
        hintText:
            HintHelper.getExampleHint(field.label, hint: field.description),
        onChanged: (v) {
          setState(() {});
        },
        validator: (v) {
          if (field.required && (v == null || v.trim().isEmpty))
            return 'Required';
          if (v != null && v.isNotEmpty && !ValidationUtils.isValidPhone(v))
        ...
        children: [
          ...
          TextFormField(
            ...
            obscureText: field.type == 'password'
                ? !(_passwordVisibility[pathKey] ?? false)
                : false,
            style: GoogleFonts.inter(
                fontSize: 13,
            ...
            onChanged: (v) {
              setState(
                  () {}); // Trigger rebuild for dynamic visibility conditions
            },
            ...
            decoration: InputDecoration(
              hintText: HintHelper.getExampleHint(field.label,
              ...
              focusedBorder: OutlineInputBorder(
                  ...
                  borderSide: const BorderSide(
                      color: AppTheme.corporateBlue, width: 1.5)),
              contentPadding:
                  const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
              suffixIcon: field.type == 'date'
                  ? const Icon(Icons.calendar_today,
                      size: 20, color: Colors.grey)
                  : (field.type == 'pa  ssword'
                      ? IconButton(
                          icon: Icon(
                              (_passwordVisibility[pathKey] ?? false)
                                  ? Icons.visibility
            ...
            validator: (v) {
              ...
              if (v != null && v.isNotEmpty) {
                ...
                bool hasPan = RegExp(r'\bpan\b', caseSensitive: false)
                        .hasMatch(field.name) ||
                    RegExp(r'\bpan\b', caseSensitive: false)
                        .hasMatch(field.label);
                bool isNotNameOrDate = !(RegExp(r'name|date|dob|first|last',
                            caseSensitive: false)
                        .hasMatch(field.name) ||
                    RegExp(r'name|date|dob|first|last', caseSensitive: false)
                        .hasMatch(field.label));

                if (hasPan && isNotNameOrDate) {
                  if (!RegExp(r'^[A-Z]{5}[0-9]{4}[A-Z]{1}$').hasMatch(strVal))
                    return 'Invalid PAN format';
                } else if (lowerName.contains('aadhaar') ||
                    lowerLabel.contains('aadhaar')) {
                  if (!RegExp(r'^\d{12}$').hasMatch(strVal))
                  ...
                  if (!RegExp(r'^\d{10}$').hasMatch(strVal))
                    ...
                    lowerLabel.contains('mail')) {
                  if (!ValidationUtils.isValidEmail(strVal))
                    return 'Invalid email address';
                } else if (RegExp(
                            r'\bname\b|\bfirst\b|\blast\b|\bcompany\b|\bbusiness\b')
                        .hasMatch(lowerName) ||
                    RegExp(r'\bname\b|\bfirst\b|\blast\b|\bcompany\b|\bbusiness\b')
                        .hasMatch(lowerLabel)) {
                  if (!lowerName.contains('company') &&
                      !lowerLabel.contains('company') &&
                      !lowerName.contains('business') &&
                      !lowerLabel.contains('business')) {
                    if (RegExp(r'[0-9]').hasMatch(strVal))
                      return 'Name cannot contain numbers';
                    if (RegExp(r'[^a-zA-Z\s\.\-]').hasMatch(strVal))
                      return 'Name cannot contain special characters';
                  } else {
                    if (RegExp(r'[^a-zA-Z0-9\s\.\-]').hasMatch(strVal))
                      return 'Business name cannot contain special characters';
                    if (!RegExp(r'[a-zA-Z]').hasMatch(strVal))
                      return 'Business name must contain at least one letter';
                  }
                } else if (lowerName.contains('place') ||
                    lowerLabel.contains('place')) {
                  if (RegExp(r'[^a-zA-Z0-9\s\.\-]').hasMatch(strVal))
                    return 'Place cannot contain special characters';
                  if (!RegExp(r'[a-zA-Z]').hasMatch(strVal))
                    return 'Place must contain at least one letter';
                }
              }
              return null;
            },
          ...
          if (hasExisting)
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: AppTheme.corporateBlue.withOpacity(0.3)),
                borderRadius: BorderRadius.circular(8),
              ),
              child: Row(
                children: [
              ...
              children: [
                const SizedBox(
                    ...
                    child: CircularProgressIndicator(
                        strokeWidth: 2, color: AppTheme.corporateBlue)),
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: _isLoading
            ...
            : _schema == null
                ? Center(
                    child: Text("Form schema not available.",
                        style: GoogleFonts.inter(
