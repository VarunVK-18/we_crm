...
class OpcFormScreen extends ConsumerStatefulWidget {
  final ServiceOrder order;
  const OpcFormScreen({super.key, required this.order});

  @override
  ConsumerState<OpcFormScreen> createState() => _OpcFormScreenState();
}

class _OpcFormScreenState extends ConsumerState<OpcFormScreen> {
  // Company Fields
  ...
  void initState() {
    ...
    _loadDraft();
    // OPC always has exactly 1 Director and 1 Nominee (Total 2 persons)
    _directors = List.generate(2, (_) => DirectorFormData());
  }
  ...
  Future<void> _loadDraft() async {
    ...
    final draft = await draftService.loadDraft(widget.order.id, 'OpcFormScreen');
    if (draft != null) {
      if (mounted) {
        setState(() {
  ...
  Future<void> _submitDetails() async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (!mounted) return;
        ref.read(draftServiceProvider).clearDraft(widget.order.id, 'OpcFormScreen');
  ...
  void _autoFillFromProfile() {
    ...
    if (user == null) return;
    setState(() {
      if (_ownerNameController.text.isEmpty && user.name.isNotEmpty) _ownerNameController.text = user.name;
      if (_companyEmailController.text.isEmpty && user.email.isNotEmpty) _companyEmailController.text = user.email;
      if (_companyPhoneController.text.isEmpty && user.phone.isNotEmpty) _companyPhoneController.text = user.phone;
    });
  }
          ...
          : Form(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  // --- Director & Nominee Details Section ---
                  ..._directors.asMap().entries.map((entry) {
                    final index = entry.key;
                    final data = entry.value;
                    final title = index == 0 ? 'Step 2: Director Details' : 'Step 3: Nominee Details';
                    final personLabel = index == 0 ? 'Director Registration' : 'Nominee Registration';
                    
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(title, style: GoogleFonts.outfit(fontSize: 22, fontWeight: FontWeight.w800, color: AppTheme.corporateBlue)),
                        const SizedBox(height: 16),
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  Text(personLabel, style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.w800, color: AppTheme.deepTeal)),
                                ],
                              ),
