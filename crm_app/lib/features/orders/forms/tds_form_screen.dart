...
class TdsFormScreen extends ConsumerStatefulWidget {
  ...
  const TdsFormScreen({super.key, required this.order});
...
class _TdsFormScreenState extends ConsumerState<TdsFormScreen> {
  ...
  Future<void> _loadDraft() async {
    ...
    final draft = await draftService.loadDraft(widget.order.id, 'TdsFormScreen');
    if (draft != null) {
      if (mounted) {
        setState(() {
  ...
  Future<void> _saveDraft() async {
    ...
    await draftService.saveDraft(widget.order.id, 'TdsFormScreen', data);
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
  ...
  Future<void> _submitDetails() async {
    ...
    if (_panCardPath == null || _addressProofPath == null || _businessAddressProofPath == null) {
    ...
    try {
      ...
      final uri = Uri.parse('$kBaseUrl/api/orders/${widget.order.id}/submit-tds-form');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        await showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: const Text('TDS Form submitted successfully!'),
            actions: [
              TextButton(
        ...
        if (!mounted) return;
        ref.read(draftServiceProvider).clearDraft(widget.order.id, 'TdsFormScreen');
  ...
  void _autoFillFromProfile() {
    ...
    if (user == null) return;
    setState(() {
      if (_emailIdController.text.isEmpty && user.email.isNotEmpty) _emailIdController.text = user.email;
      if (_mobileNumberController.text.isEmpty && user.phone.isNotEmpty) _mobileNumberController.text = user.phone;
    });
  }
            ...
            isExpanded: true,
                  ...
                  _buildSectionContainer(
                    ...
                    children: [
                      ...
                      _buildFileRow('Address Proof', 'Authorized Signatory Proof. Max 2 MB.', _addressProofPath, () => _pickFile((path) => _addressProofPath = path, allowedExtensions: const ['pdf'])),
                      ...
                      _buildFileRow('Incorporation Certificate', 'Only for Company/LLP. Max 2 MB.', _incorpCertPath, () => _pickFile((path) => _incorpCertPath = path), isRequired: false),
                    ],
                  ),
